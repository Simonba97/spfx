 ...
 *
 * @export
 * @class TabsInvormativosServices
 * @extends {ListItemService}
 */
export class TabsInvormativosServices extends ListItemService {
    private _mycontext: WebPartContext | ApplicationCustomizerContext;

    /**
     * Crear una instancia de TabsInvormativosServices
     * @param {string} listName
     * @param {(WebPartContext | ApplicationCustomizerContext)} context
     * @memberof TabsInvormativosServices
     */
    ...
    */
    public async getLatestItemId(): Promise<number> {
        ...
        //Opcion 1
        const items: ITabsInformativos[] = await this.getItems(query);
        //Opcion 2
        //const items: ITabsInformativos[] = await sp.web.lists.getByTitle(this.getListName()).items.orderBy('Id', false).top(1).select('Id').get();
